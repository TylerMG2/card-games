use crate::{
    games::{carbo, coup, tycoon},
    helpers::{is_host, is_lobby},
    traits::{GameLogic, GameSignal, Networking},
    types::{self, ClientEvent, CommonClientEvent, CommonServerEvent, ServerEvent},
};
        ...
        ServerEvent::CommonEvent(event) => {
            match event {
                CommonServerEvent::HostChanged { player_index } => {
                    // TODO: After finishing the game, we may be able to remove this event and just have the host change when the host disconnects
...
pub fn handle_client_event(
    ...
    match event {
        ...
        ClientEvent::CommonEvent(event) => match event {
            CommonClientEvent::LeaveRoom => {
                connections.send_to_all_except_origin(
                    room,
                    ServerEvent::CommonEvent(CommonServerEvent::PlayerLeft {
            ...
            CommonClientEvent::ChangeGame { game } => {
                connections.send_to_all_except_origin(
                    room,
                    ServerEvent::CommonEvent(CommonServerEvent::GameChanged { game: *game }),
                    player_index,
                );
            }
            ...
            CommonClientEvent::ResetGame => {
                connections.send_to_all_except_origin(
                    room,
                    ServerEvent::CommonEvent(CommonServerEvent::ResetGame),
                    player_index,
                );
            }
        },
